...
export async function onRequestPost({ request, env }) {
  ...
  const invite = code ? await env.DB.prepare(`SELECT i.id AS invite_id,i.server_id,i.max_uses,i.uses,i.expires_at,i.revoked_at,s.id,s.name,s.description,s.icon_text,s.icon_color,s.is_public,s.owner_user_id FROM server_invites i JOIN servers s ON s.id=i.server_id WHERE i.code=? LIMIT 1`).bind(code).first() : null;
  ...
  return json({ ok: true, server: { id:server.id,name:server.name,description:server.description,icon_text:server.icon_text,icon_color:server.icon_color,is_public:Boolean(server.is_public),owner_user_id:server.owner_user_id,role:"member" } });
}
