...
export async function onRequestPatch({request,env}){
 ...
 let iconData=d.icon_data===undefined?current.icon_data:String(d.icon_data||"");if(iconData&&(!iconData.startsWith("data:image/")||iconData.length>700000))return json({ok:false,error:"Invalid server icon"},{status:400});
 ...
 await env.DB.prepare(`UPDATE servers SET name=?,description=?,icon_color=?,icon_data=?,is_public=?,rules_text=?,verification_level=?,default_notifications=? WHERE id=?`).bind(name,description,iconColor,iconData||null,isPublic,rulesText,verificationLevel,defaultNotifications,serverId).run();
 return json({ok:true,server:{id:serverId,name,description,icon_color:iconColor,icon_data:iconData||null,is_public:Boolean(isPublic),rules_text:rulesText,verification_level:verificationLevel,default_notifications:defaultNotifications}});
}
export async function onRequestDelete({request,env}){
 const user=await getCurrentUser(request,env);if(!user)return json({ok:false,error:"Not authenticated"},{status:401});let d={};try{d=await request.json();}catch{}const serverId=String(d.server_id||""),m=await mine(env,serverId,user.id);if(!m)return json({ok:false,error:"Server not found"},{status:404});
 if(m.role==="owner"){const s=await env.DB.prepare(`SELECT name FROM servers WHERE id=?`).bind(serverId).first();if(String(d.confirm_name||"")!==s?.name)return json({ok:false,error:"Server name does not match"},{status:400});await env.DB.prepare(`DELETE FROM servers WHERE id=?`).bind(serverId).run();return json({ok:true,deleted:true,server_id:serverId});}
